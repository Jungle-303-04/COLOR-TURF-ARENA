...
관리자 `게임·봇 제어` 탭에서 50·100·250·500개 프리셋 또는 직접 입력한 수만큼 봇을 한 번에 추가·회수할 수 있다. 각 봇은 서버 내부 숫자가 아니라 별도 Socket.IO client로 실제 `join_room`과 `player_input` 프로토콜에 접속한다. `운영 지표` 탭에서는 선택한 방의 Stable/Canary authority에서 입력 처리량, 입력/Tick/Event Loop 지연, Socket, CPU, RSS Memory, 상태 Payload를 가져와 최근 120초 시계열로 표시한다. 각 지표 이름 옆 `?`는 정의와 실제 수집 API·런타임 원천을 표시한다.
...
2026-07-16 기본 측정에서는 두 모드 모두 입력 `2,500/2,500`, 거부 `0`이었고 Full은 Delta보다 대표 Payload `5.49배`, 서버 Payload P95 `5.30배`, Broadcast P95 `1.60배`, Event Loop P95 `1.57배`, CPU `4.36배`를 사용했다. 실행법과 필드 정의는 [Delta / Full Broadcast 부하 비교](docs/load-comparison.md)에 있다.
...
Playwright E2E는 빈 임시 포트에 메모리 기반 API와 Vite를 직접 시작하고 테스트 종료 시 모두 닫는다. Chromium에서 관리자 방 생성, 참가자 닉네임 입장, 경기 시작, 실제 Pointer 조이스틱 입력, 관전 점수 반영, Paint Boost의 참가자·관전 표시, offline → online 자동 재접속과 동일 Session·닉네임·팀 복구를 한 흐름으로 검증한다.
...
`values-primary.yaml`은 공개 환경 기준으로 관리자 인증을 유지하고 실제 OOM 주입을 비활성화한다. 인증 우회와 OOM 주입을 동시에 허용하는 `values-isolated-chaos-demo.yaml`은 외부 Ingress/Tunnel이 없는 폐쇄형 일회성 namespace에서만 추가한다.
