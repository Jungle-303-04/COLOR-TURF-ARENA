...
| URL | 용도 |
| --- | --- |
| `/admin` | 토큰 로그인, `게임 진행 상황`·`게임·봇 제어`·`운영 지표` 탭, 실시간 관전과 최대 120초 운영 그래프 |
...
관리자 `게임·봇 제어` 탭에서 50·100·250·500개 프리셋 또는 직접 입력한 수만큼 봇을 한 번에 추가·회수할 수 있다. 각 봇은 서버 내부 숫자가 아니라 별도 Socket.IO client로 실제 `join_room`과 `player_input` 프로토콜에 접속한다. `운영 지표` 탭에서는 선택한 방의 Stable/Canary authority에서 입력 처리량, 입력/Tick/Event Loop 지연, Socket, CPU, RSS Memory, 상태 Payload와 플레이·관전 브라우저의 실제 FPS·프레임 누락률을 가져와 최근 최대 120초 시계열로 표시한다. 각 지표 이름 옆 `?`는 정의와 실제 수집 API·런타임 원천을 표시한다.
